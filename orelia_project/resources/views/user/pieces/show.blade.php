...
@else
    <div class="row g-4">
        ...
        <div class="col-md-7">
            ...
            <h2 class="fw-bold mb-1">{{ $viewData['piece']->getName() }}</h2>
            <span class="badge bg-secondary mb-3">{{ $viewData['piece']->getType() }}</span>

            <p class="text-muted">{{ $viewData['piece']->getDescription() }}</p>
            ...
            @if($viewData['piece']->getStock() > 0)
                ...
                <form action="{{ route('cart.add', $viewData['piece']->getId()) }}" method="POST" class="d-inline">
                    @csrf
                    <button type="submit" class="btn btn-dark mt-2">{{ __('pieces.add_to_cart') }}</button>
                </form>
            @else
