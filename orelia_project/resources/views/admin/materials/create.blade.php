                ...
                <form action="{{ route('admin.materials.store') }}" method="POST">
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select @error('type') is-invalid @enderror"
                            ...
                            <option value="metal" {{ old('type') == 'metal' ? 'selected' : '' }}>{{ __('materials.type_metal') }}</option>
                            <option value="gemstone" {{ old('type') == 'gemstone' ? 'selected' : '' }}>{{ __('materials.type_gemstone') }}</option>
                            <option value="leather" {{ old('type') == 'leather' ? 'selected' : '' }}>{{ __('materials.type_leather') }}</option>
                            <option value="textile" {{ old('type') == 'textile' ? 'selected' : '' }}>{{ __('materials.type_textile') }}</option>
                            <option value="other" {{ old('type') == 'other' ? 'selected' : '' }}>{{ __('materials.type_other') }}</option>
                        </select>
                        @error('type')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    ...
                    <div class="d-flex gap-2">
                        <button type="submit" class="btn btn-dark">{{ __('materials.save') }}</button>
